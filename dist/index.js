...
async function generateContextualResponse(message, conversation, stage) {
...
function generateResponseBasedOnContext(message, stage, context) {
...
async function createProjectPlanFromConversation(conversation) {
  ...
  if (complexityFactors > 3) {
    ...
    budget = budget.replace(/\$(\d+),(\d+) - \$(\d+),(\d+)/, (match, s1, s2, e1, e2) => `$${parseInt(s1) + 5},${s2} - $${parseInt(e1) + 10},${e2}`);
  }
  const plan = {
...
async function registerRoutes(app2) {
  app2.post("/api/elevenlabs/chat", chat);
  app2.post("/api/elevenlabs/text-to-speech", textToSpeech);
  app2.post("/api/elevenlabs/speech-to-text", speechToText);
  app2.post("/api/elevenlabs/generate-plan", generateProjectPlan);
