...
export class BrowserVectorRAG {
  ...
  async processDocument(file: File): Promise<VectorSearchResult> {
    ...
    try {
      console.log(
        "🔍 Processing document with browser vector search:",
        file.name
      );
  ...
  private storeDocumentChunks(chunks: DocumentChunk[]): void {
    ...
    try {
      ...
      const filteredExisting = existingChunks.filter(
        (chunk) => chunk.documentId !== chunks[0]?.documentId
      );
  ...
  private async extractKeyPointsWithSearch(
    documentName: string
  ): Promise<KeyPoint[]> {
    ...
    for (const query of keyPointQueries) {
      try {
        ...
        const similarChunks = this.performSimilaritySearch(
