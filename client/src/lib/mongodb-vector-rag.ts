...
export interface DocumentChunk {
  ...
  metadata: {
    ...
    keyScore: number; // Relevance score for key point extraction
  };
}
...
export class MongoDBVectorRAG {
  ...
  private db!: Db;
  private collection!: Collection<DocumentChunk>;
  ...
  constructor() {
    const connectionString = "mongodb+srv://bd1:Papun$1996@cluster0.mehhr.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0&readPreference=primary";
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.db = this.client.db('brand_guidelines_rag');
      this.collection = this.db.collection<DocumentChunk>('document_chunks');
  ...
  private async ensureVectorIndex(): Promise<void> {
    try {
      ...
      const vectorIndexExists = indexes.some(index => index.name === 'vector_search_index');
      
      if (!vectorIndexExists) {
        console.log('Creating text search index...');
        await this.collection.createIndex(
          { "content": "text", "metadata.keyScore": 1 },
