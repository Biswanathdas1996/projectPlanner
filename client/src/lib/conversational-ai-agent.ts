...
export class ConversationalAIAgent {
  ...
  private currentStage: 'discovery' | 'analysis' | 'specification' | 'planning' | 'complete' = 'discovery';
...
`;
  ...
  private parseAnalysisResponse(response: string): Partial<ConversationContext> {
    ...
    if (response.toLowerCase().includes('web')) context.projectType = 'web application';
    if (response.toLowerCase().includes('mobile')) context.projectType = 'mobile application';
    if (response.toLowerCase().includes('ecommerce') || response.toLowerCase().includes('shop')) {
      context.problemDomain = 'e-commerce';
    }
    if (response.toLowerCase().includes('healthcare')) context.problemDomain = 'healthcare';
...
Recent conversation:
${this.conversation.slice(-4).map(msg => `${msg.role}: ${msg.content}`).join('\n')}
...
`;
  ...
  private parseAgentResponse(response: string): AgentResponse {
    try {
      const messageMatch = response.match(/MESSAGE:\s*([\s\S]*?)(?=NEXT_QUESTIONS:|$)/);
      const questionsMatch = response.match(/NEXT_QUESTIONS:\s*([\s\S]*?)(?=CONFIDENCE:|$)/);
      const confidenceMatch = response.match(/CONFIDENCE:\s*(\d+)/);
      const shouldGeneratePlanMatch = response.match(/SHOULD_GENERATE_PLAN:\s*true/);

      const message = messageMatch ? messageMatch[1].trim() : response;
      const nextQuestions = questionsMatch ? this.parseQuestions(questionsMatch[1]) : [];
  ...
  private updateStage(): void {
    ...
    } else if (this.conversation.length >= 8 && this.currentStage === 'specification') {
      this.currentStage = 'planning';
    } else if (this.conversation.length >= 12 && this.currentStage === 'planning') {
  ...
  private assessContextCompleteness(): number {
    ...
    const requiredScore = requiredFields.filter(field => this.context[field as keyof ConversationContext]).length / requiredFields.length;
    const optionalScore = optionalFields.filter(field => this.context[field as keyof ConversationContext]).length / optionalFields.length;
  ...
  private determineIntent(message: string): string {
    ...
    if (lowerMessage.includes('help') || lowerMessage.includes('need')) return 'request_help';
    if (lowerMessage.includes('build') || lowerMessage.includes('create')) return 'build_request';
    if (lowerMessage.includes('cost') || lowerMessage.includes('price') || lowerMessage.includes('budget')) return 'budget_inquiry';
    if (lowerMessage.includes('time') || lowerMessage.includes('when') || lowerMessage.includes('timeline')) return 'timeline_inquiry';
    if (lowerMessage.includes('technology') || lowerMessage.includes('tech') || lowerMessage.includes('stack')) return 'tech_inquiry';
...
`;
  ...
  private validateProjectPlan(plan: any): ProjectPlan {
    return {
      title: plan.title || `${this.context.projectType || 'Custom'} Development Project`,
      description: plan.description || 'A custom solution tailored to your specific requirements.',
      phases: Array.isArray(plan.phases) ? plan.phases : this.createDefaultPhases(),
      technologies: Array.isArray(plan.technologies) ? plan.technologies : this.getRecommendedTechnologies(),
      ...
      recommendations: Array.isArray(plan.recommendations) ? plan.recommendations : this.getRecommendations(),
    };
  }
  ...
  private createFallbackPlan(): ProjectPlan {
    return {
      ...
      description: 'A tailored solution based on your requirements and our consultation.',
    ...
    return [
      {
        ...
        duration: '2-3 weeks',
        tasks: ['Requirements analysis', 'Technical architecture', 'Project planning'],
      ...
      },
      {
        ...
        deliverables: ['Design system', 'Interactive prototype', 'User feedback report']
      },
      {
        name: 'Development',
        duration: '6-10 weeks',
  ...
  private assessProjectComplexity(): number {
    ...
    if (this.context.techStack && this.context.techStack.length > 5) complexity += 0.2;
    if (this.context.requirements && this.context.requirements.length > 10) complexity += 0.2;
    if (this.context.stakeholders && this.context.stakeholders.length > 3) complexity += 0.1;
    if (this.context.problemDomain === 'healthcare' || this.context.problemDomain === 'finance') complexity += 0.2;
