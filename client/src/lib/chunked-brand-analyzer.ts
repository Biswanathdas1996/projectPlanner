...
export class ChunkedBrandAnalyzer {
  ...
  async analyzeExtractedContent(
    report: ComprehensiveBrandReport
  ): Promise<FinalBrandReport> {
    ...
    try {
      ...
      const brandGuidelines = await this.consolidateBrandGuidelines(
        chunkedAnalyses,
        report
      ).catch((error) => {
      ...
      const keyFindings = await this.extractKeyFindings(chunkedAnalyses).catch(
        (error) => {
      ...
      const averageConfidence =
        chunkedAnalyses.reduce((sum, chunk) => sum + chunk.confidence, 0) /
        chunkedAnalyses.length;
      ...
      console.log("✅ Chunked analysis completed:", {
        ...
        confidence: Math.round(
          finalReport.documentInfo.averageConfidence * 100
        ),
  ...
  private createContentChunks(
    ...
    const colorPages = pageExtractions.filter(
      (page) =>
        page.textContent.toLowerCase().includes("color") ||
        page.visualElements.colors.length > 0
    );

    const typographyPages = pageExtractions.filter(
      (page) =>
    ...
    const logoPages = pageExtractions.filter(
      (page) =>
        page.textContent.toLowerCase().includes("logo") ||
        page.visualElements.logos.length > 0
    );

    const layoutPages = pageExtractions.filter(
      (page) =>
    ...
    const remainingPages = pageExtractions.filter(
      (page) =>
    ...
    if (remainingPages.length > 0) {
      chunks.push({
        id: "brand-voice",
        content: this.consolidatePageContent(
          remainingPages,
  ...
  private consolidatePageContent(
  ...
  private async analyzeChunks(
    ...
    for (const chunk of chunks) {
      ...
      try {
        ...
        console.log(
          `✅ ${chunk.id} chunk analyzed with confidence ${analysis.confidence}`
        );
      } catch (error) {
  ...
  private async analyzeChunk(chunk: {
    id: string;
    content: string;
  }): Promise<{ guidelines: BrandGuideline[]; confidence: number }> {
...
Output ONLY valid JSON, no additional text.`;
  ...
  private createFallbackAnalysis(chunk: {
    id: string;
    content: string;
  }): ChunkedAnalysis {
  ...
  private extractSpecifications(content: string): string[] {
    const specs =
      content.match(/\d+px|\d+%|#[0-9A-Fa-f]{6}|[A-Z][a-z]+ [A-Z][a-z]+/g) ||
      [];
  ...
  private extractRestrictions(content: string): string[] {
    const restrictions =
      content.match(/not[^.]*|never[^.]*|avoid[^.]*|don't[^.]*/gi) || [];
    return restrictions.slice(0, 3);
  }

  private async consolidateBrandGuidelines(
    analyses: ChunkedAnalysis[],
    originalReport: ComprehensiveBrandReport
  ): Promise<FinalBrandReport["brandGuidelines"]> {
    const colorGuidelines =
      analyses.find((a) => a.chunkId === "colors")?.extractedGuidelines || [];
    const typographyGuidelines =
      analyses.find((a) => a.chunkId === "typography")?.extractedGuidelines ||
      [];
    const logoGuidelines =
      analyses.find((a) => a.chunkId === "logos")?.extractedGuidelines || [];
    const layoutGuidelines =
      analyses.find((a) => a.chunkId === "layout")?.extractedGuidelines || [];
    const brandVoiceGuidelines =
      analyses.find((a) => a.chunkId === "brand-voice")?.extractedGuidelines ||
      [];
    ...
    return {
      ...
      typography: {
        ...
        headingStyles: this.extractTypographyValues(
      ...
      tone: {
        personality: this.extractBrandVoiceValues(
          brandVoiceGuidelines,
          "personality"
        ),
        voice: this.extractBrandVoiceValues(brandVoiceGuidelines, "voice"),
        messaging: this.extractBrandVoiceValues(
          brandVoiceGuidelines,
          "messaging"
        ),
        doAndDonts: {
  ...
  private extractTypographyValues(
    guidelines: BrandGuideline[],
    type: string
  ): string[] {
    const allContent = guidelines.flatMap((g) => [
      ...g.specifications,
      ...g.usage,
      g.description,
    ]);
  ...
  private extractLogoValues(
    ...
    if (type === "variations") {
      ...
      return variations.filter((v) =>
        allContent.some((content) => content.toLowerCase().includes(v))
      );
    }

    if (type === "usage") {
      return allContent
        .filter((item) => item.includes("use") || item.includes("place"))
        .slice(0, 3);
    }
  ...
  private extractLayoutValues(
    ...
    if (type === "grid") {
      ...
      return gridTerms.filter((term) =>
        allContent.some((content) => content.toLowerCase().includes(term))
      );
    }
  ...
  private extractBrandVoiceValues(
    guidelines: BrandGuideline[],
    type: string
  ): string[] {
    const allContent = guidelines.flatMap((g) => [
    ...
    switch (type) {
      ...
      case "dos":
        return allContent
          .filter((item) => item.includes("should") || item.includes("use"))
          .slice(0, 3);
      case "donts":
        return allContent
          .filter((item) => item.includes("avoid") || item.includes("never"))
          .slice(0, 3);
      default:
        return [];
    }
  ...
  private extractAccessibilityValues(analyses: ChunkedAnalysis[]): string[] {
    ...
    return accessibilityTerms
      .filter((term) => allContent.includes(term))
      .map(
        (term) => `${term.charAt(0).toUpperCase() + term.slice(1)} requirements`
      );
  }

  private createFallbackBrandGuidelines(
    analyses: ChunkedAnalysis[]
  ): FinalBrandReport["brandGuidelines"] {
    return {
      colors: {
  ...
  private createFallbackKeyFindings(
    analyses: ChunkedAnalysis[]
  ...
  private async extractKeyFindings(
    analyses: ChunkedAnalysis[]
  ): Promise<FinalBrandReport["keyFindings"]> {
    const allGuidelines = analyses.flatMap((a) => a.extractedGuidelines);
    const criticalGuidelines = allGuidelines.filter(
