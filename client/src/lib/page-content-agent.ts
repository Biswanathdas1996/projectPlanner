...
export class PageContentAgent {
  ...
  async generatePageContent(
    request: PageContentGenerationRequest
  ): Promise<PageContentCard[]> {
    try {
      ...
      throw new Error(
        "Failed to generate page content: " + (error as Error).message
      );
    }
  }

  async generateSinglePageContent(
    pageReq: any,
    context: {
      stakeholderFlows: any[];
      flowTypes: any;
...
Generate realistic, relevant content that matches the page purpose and stakeholder needs. Make the content specific to the ${
      pageReq.pageType
    } page type and ${pageReq.purpose} purpose.
`;
  }
  ...
  parseContentResponse(
    response: string,
    pageReq: any
  ): Omit<PageContentCard, "id" | "isEdited"> {
    try {
      ...
      return {
        ...
        headers: Array.isArray(parsed.headers)
          ? this.sanitizeStringArray(parsed.headers)
          : this.generateFallbackHeaders(pageReq),
        textContent: Array.isArray(parsed.textContent)
          ? this.sanitizeStringArray(parsed.textContent)
          : this.generateFallbackTextContent(pageReq),
        buttons: Array.isArray(parsed.buttons)
          ? this.validateButtons(parsed.buttons)
          : this.generateFallbackButtons(pageReq),
        forms: Array.isArray(parsed.forms)
          ? this.validateForms(parsed.forms)
          : this.generateFallbackForms(pageReq),
        inputs: Array.isArray(parsed.inputs)
          ? this.validateInputs(parsed.inputs)
          : this.generateFallbackInputs(pageReq),
        lists: Array.isArray(parsed.lists)
          ? this.validateLists(parsed.lists)
          : this.generateFallbackLists(pageReq),
        images: Array.isArray(parsed.images)
          ? this.validateImages(parsed.images)
          : this.generateFallbackImages(pageReq),
        navigation: Array.isArray(parsed.navigation)
          ? this.sanitizeStringArray(parsed.navigation)
          : this.generateFallbackNavigation(pageReq),
        additionalContent: Array.isArray(parsed.additionalContent)
          ? this.sanitizeStringArray(parsed.additionalContent)
          : this.generateFallbackAdditionalContent(pageReq),
      };
    } catch (error) {
  ...
  private generateFallbackHeaders(pageReq: any): string[] {
    return [pageReq.pageName, "Overview", "Details", "Actions"];
  }
  ...
  private generateFallbackTextContent(pageReq: any): string[] {
    return [
      ...
      `This page helps you ${
        pageReq.purpose?.toLowerCase() || "manage your tasks"
      }`,
  ...
  private generateFallbackButtons(
    pageReq: any
  ...
  private generateFallbackForms(
    pageReq: any
  ): { title: string; fields: string[]; submitAction: string }[] {
    const type = pageReq.pageType?.toLowerCase() || "form";
  ...
  private generateFallbackInputs(
    pageReq: any
  ...
  private generateFallbackLists(
    pageReq: any
  ): { title: string; items: string[]; type: string }[] {
    return [
      {
        title: "Recent Items",
  ...
  private generateFallbackImages(
    pageReq: any
  ): { alt: string; description: string; position: string }[] {
    return [
      {
        alt: "Header image",
      ...
      {
        alt: "Content image",
        description: "Main content illustration",
        position: "center",
      },
  ...
  private generateFallbackNavigation(pageReq: any): string[] {
    return ["Home", "Dashboard", "Profile", "Settings", "Help", "Logout"];
  }
  ...
  private validateForms(
    ...
    return forms.map((form) => ({
      ...
      fields: Array.isArray(form.fields)
        ...
        : ["Field 1", "Field 2"],
      submitAction: String(
        form.submitAction || form.action || form.onSubmit || "submit"
      ),
    }));
  }
    ...
    return lists.map((list) => ({
      ...
      items: Array.isArray(list.items)
        ? list.items.map((item: any) =>
            typeof item === "string"
              ? item
              : String(item.text || item.label || item.name || "Item")
          )
        : ["Item 1", "Item 2"],
